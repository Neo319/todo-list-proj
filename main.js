...
const { format, addDays} = __webpack_require__(/*! date-fns */ "./node_modules/date-fns/index.js");
...
const loadItem = (allProjects, project, item) => {
    ...
    dueDate.addEventListener('click', function dateClicked () {
        setDate(dueDate, dateClicked, function (userInput) {
            ...
            enteredDate = addDays(enteredDate, 1) // correcting for time Zones 
            const formattedDate = format(enteredDate, 'MMMM dd, yyyy');

            (0,_app_logic_projectManager__WEBPACK_IMPORTED_MODULE_1__["default"])(allProjects).setNewDate(project, item, formattedDate);
        });
    })



    titleDiv.appendChild(dueDate);
    ...
    function setDate (location, source, callback) {
        const message = document.createElement('h5');
        message.textContent = 'Please enter a new due date.';
        const dateInput = document.createElement('input');
        dateInput.type = 'date';
        const focusDiv = document.createElement('div');
        
        focusDiv.appendChild(message);
        focusDiv.appendChild(dateInput);
        ...
        //button for submitting more intuitively
        ...
        focusDiv.appendChild(submitBtn);

        //button for closing more intuitively
        const closeBtn = document.createElement('button');
        closeBtn.textContent = 'X'
        focusDiv.appendChild(closeBtn);

        closeBtn.addEventListener('click', () => { //element will be removed and event listeners restored
            location.removeChild(focusDiv);
            location.addEventListener('click', source);
        });
        location.appendChild(focusDiv);
...
const populatePage = (allProjects) => {
    ...
    hero.textContent = "JavaScript Jotter";
    const heroSubText = document.createElement('span');
    heroSubText.textContent = "A project by Alex Nevins"; 
    hero.classList = "hero";
    
    
    header.appendChild(hero);
    header.appendChild(heroSubText);

    let loadedProject = allProjects[0]; // which project is currently loaded in the main window
        ...
        allProjects.forEach(project => {
            ...
            let myItemsList = document.createElement('ul');
                ...
                project.forEach(item => { // loop to display the title of each ITEM
                    if (item.title) { 
                        ...
                        removeItemBtn.addEventListener('click', () => {
                           ...
                           createSidebarList();
                           if (loadedProject === project) {reloadMainWindow(project)}; //reload window when an item is deleted if the same project is loaded
                        });
                ...
                addItemBtn.addEventListener('click', () => {
                    ...
                    createSidebarList();
                    if (loadedProject === project) {reloadMainWindow(project)}; 
                });
            ...
            if (project[0] !== 'defaultProject') {
                ...
                removeProjBtn.addEventListener('click', () => {
                    ...
                    if (loadedProject === project) {
                        ...
                        loadedProject = allProjects[0];
                        reloadMainWindow(allProjects[0]);
                    }
                })
...
const { format } = __webpack_require__(/*! date-fns */ "./node_modules/date-fns/index.js");
    ...
    class ListItem {
        constructor(title, description, dueDate, priority, completed) {
            ...
            if (dueDate === 'today') { //handy shortcut
